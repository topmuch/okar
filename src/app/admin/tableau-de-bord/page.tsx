...
interface RecentActivity {
  id: string;
  type: string; // 'activation' | 'order' | 'scan' | 'registration' | 'payment' | etc.
  name: string;
  reference: string;
  time: string;
  details: string;
  status: string; // 'success' | 'warning' | 'info' | 'pending' | 'error' | etc.
  agency?: string;
}
...
function ActivityItem({ activity }: { activity: RecentActivity }) {
  const statusConfig: Record<string, { bg: string; icon: React.ReactNode }> = {
    ...
    pending: { bg: 'bg-orange-100 dark:bg-orange-500/10', icon: <Clock className="w-4 h-4 text-orange-500" /> },
    error: { bg: 'bg-red-100 dark:bg-red-500/10', icon: <Package className="w-4 h-4 text-red-500" /> },
    // Default fallback for unknown status types
    default: { bg: 'bg-slate-100 dark:bg-slate-500/10', icon: <Clock className="w-4 h-4 text-slate-500" /> }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Vue d'ensemble de votre activité OKAR</p>
      </div>
