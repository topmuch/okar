...
export async function GET(request: NextRequest) {
  try {
    ...
    const records = rawRecords.map(r => ({
      ...
      // Major repair fields
      isMajorRepair: r.isMajorRepair === 1 || r.isMajorRepair === true,
      affectedOrgans: r.affectedOrgans,
      partCondition: r.partCondition,
      accidentRelated: r.accidentRelated === 1 || r.accidentRelated === true,
      accidentDescription: r.accidentDescription,
      repairPhotos: r.repairPhotos,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      paperDocumentUrl,
      isVerified = false,
      // Structured maintenance details
      maintenanceDetails,
      // Major repair fields
      isMajorRepair = false,
      affectedOrgans,
    ...
    await db.$executeRawUnsafe(`
      INSERT INTO MaintenanceRecord (
        ...
        mileage, partsList, maintenanceDetails, 
        isMajorRepair, affectedOrgans, partCondition, accidentRelated, accidentDescription, repairPhotos,
    ...
    `, 
      ...
      mileage || null,
      partsList || null,
      maintenanceDetails || null,  // Structured maintenance details (JSON)
      isMajorRepair ? 1 : 0,       // isMajorRepair
      affectedOrgans || null,      // affectedOrgans (JSON array)
      partCondition || null,       // partCondition
      accidentRelated ? 1 : 0,     // accidentRelated
      accidentDescription || null, // accidentDescription
      repairPhotos || null,        // repairPhotos (JSON array)
      partsCost || 0, 
      laborCost || 0, 
      totalCost || 0,
      invoicePhoto || null, 
