...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Vérifier que l'utilisateur est un garage ou superadmin
    if (user.role !== 'garage' && user.role !== 'superadmin') {
      return NextResponse.json({
        success: false,
        error: 'Seul un garage ou SuperAdmin peut marquer un QR code comme perdu'
      }, { status: 403 });
    }
