...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      db.$queryRaw`SELECT * FROM Garage`,
      db.$queryRaw`SELECT * FROM Vehicle`,
      db.$queryRaw`SELECT * FROM MaintenanceRecord`,
    ...
    const backup = {
      ...
      stats: {
        ...
        garages: Array.isArray(garages) ? garages.length : 0,
        vehicles: Array.isArray(vehicles) ? vehicles.length : 0,
        maintenanceRecords: Array.isArray(maintenanceRecords) ? maintenanceRecords.length : 0,
    ...
    const filename = `okar-backup-${date}.json`;
