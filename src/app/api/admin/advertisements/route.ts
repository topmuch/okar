...
export async function POST(request: NextRequest) {
  try {
    ...
    if (targetScope === 'garage' && !garageId) {
      return NextResponse.json(
        { error: 'Un garage doit être sélectionné pour le ciblage par garage' },
        { status: 400 }
      );
    }
    ...
    const advertisement = await db.advertisement.create({
      data: {
        ...
        garageId: targetScope === 'garage' ? garageId : null,
