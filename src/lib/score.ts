 ...
 * - GOLD: 70-100
 */
export async function calculateVehicleScore(vehicleId: string): Promise<ScoreResult> {
  const now = new Date();
...
/**
 * Trigger score calculation and update for a vehicle
 * This function should be called after:
 * - Validation of an intervention
 * - Addition of paper history
 * - Update of VT/Assurance documents
 */
export async function triggerScoreCalculation(vehicleId: string): Promise<ScoreResult> {
  const result = await calculateVehicleScore(vehicleId);
  ...
  await db.$executeRawUnsafe(`
    ...
    WHERE id = ?
  `, result.score, result.badge, now, now, vehicleId);
