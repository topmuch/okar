...
// User with garage info for the session
...
export async function getActiveSessions(): Promise<(Session & { user: { email: string; name: string | null; role: string } })[]> {
  ...
  // Transform to lowercase 'user' for consistent API response
 ...
 */
export async function requireAuth(allowedRole?: 'superadmin' | 'garage'): Promise<SessionUser> {
  const user = await getSession();

  if (!user) {
    const loginPath = allowedRole === 'superadmin' ? '/admin/connexion' : '/garage/connexion';
  ...
  if (allowedRole && user.role !== allowedRole) {
    ...
    if (user.role === 'superadmin') {
      ...
      throw new Error('REDIRECT:/garage/tableau-de-bord');
    }
  }
